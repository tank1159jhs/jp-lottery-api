// 미즈호은행 공식 CSV에서 과거 Loto7 전체 회차 데이터를 파싱해 최신 99회차를 저장하는 스크립트
...
async function crawlHistoricalLoto7FromCSV() {
  try {
    ...
    // 최신값에서 회차 추출 (loto7: A103)
    const latestFileName = lines[0].split('\t')[1].trim();
    const roundMatch = latestFileName.match(/A103(\d{4})\.CSV/i);
    ...
    for (let i = 0; i < 99; i++) {
      ...
      const fileName = `A103${round.toString().padStart(4, '0')}.CSV`;
      const csvUrl = `https://www.mizuhobank.co.jp/retail/takarakuji/loto/loto7/csv/${fileName}`;
      ...
      try {
        ...
        if (numbers.length !== 7 || bonus_numbers.length !== 2) throw new Error('데이터 불완전');
