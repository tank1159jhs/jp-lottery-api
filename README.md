...
### 회차별 JSON (예: `2061.json`)
```json
{
  "type": "loto6",
  "round": 2061,
  "date": "2024-12-23",
  "numbers": [1, 12, 23, 34, 35, 43],
  "bonus": 7
}
```
...
- **`latest.json`**: 가장 최근 추첨 회차 데이터 (크롤링 실패 시 이전 데이터 유지)
- **`all.json`**: 최근 100회차 목록 (배열, 회차 내림차순 정렬)
- **`{회차}.json`**: 특정 회차 데이터 (예: 2061.json)

### 데이터 관리 정책

📌 **최대 100회차 유지**: 새 회차가 추가되면 100회 이전 데이터는 자동 삭제됩니다.

⚠️ **크롤링 실패 정책**: 
- 더미 데이터 생성 금지 (잘못된 당첨 정보 방지)
- 실패 시 기존 `latest.json` 유지
- GitHub Actions 워크플로우가 실패 상태로 표시됨
- 다음 스케줄 실행 시 자동 재시도
